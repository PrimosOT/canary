...
modalBuyMW:id(CUSTOM_MW_BUY_ITEMID)
modalBuyMW:register()
...
function creatureBuyMW.onModalWindow(player, modalWindowId, buttonId, choiceId)
    if player then
		...
		if buttonId == 0 and player:getItemCount(CUSTOM_MW_BUY_ITEMID) >= 1 and mwEscolhida then
			if player:removeItem(CUSTOM_MW_BUY_ITEMID,1) then
